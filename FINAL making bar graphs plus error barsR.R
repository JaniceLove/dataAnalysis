...
#mac shortcut: to run current selection: option + Enter
#mac filepath: new finder window (command + N), command + shift +G, drag file to box

#Before running, clear global environment 
rm (list = ls())
...
#only needs to be installed once per session
...
#variable <- t.test (x, y)
#equal variance: t.test(x, y, var.equal = TRUE)
